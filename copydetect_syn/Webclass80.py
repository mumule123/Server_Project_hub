import time
from flask import Flask, request, render_template_string,send_from_directory,render_template,redirect
import re
        ...
        if request.method == 'POST':
            ...
            for i in range(len(words) - n + 1):
                ...
                if len(query_phrases) >= 5000:
                    tem_ans = []
                    should_clauses = [{"match_phrase": {"desc": phrase}} for phrase in query_phrases]
                    query_body = {
                        "_source" :{
                        },
                        "query": {
                    ...
                    for se_res in response['hits']['hits']:
                        if cc >= top_n:
                            break
                        search_arr.append({"k": str(se_res['_source']['id']), "v": str(se_res['_score']),"n": str(se_res['_source']['desc'])})
                    cc += 1
                    query_phrases = []
            should_clauses = [{"match_phrase": {"desc": phrase}} for phrase in query_phrases]
            query_body = {
            ...
            for v in search_arr:
                ...
                with open(search_folder +'/'+ pre_name + '.java','w',encoding='utf-8')as f:
                    f.write(content)
            ...
            with open(pairs_path, "w") as file:
                for v in ha.keys():
                    ...
                    file.writelines(os.path.basename(file_path)+" "+seven_digit_number+".java\n")
            # 跑程序得到结果
